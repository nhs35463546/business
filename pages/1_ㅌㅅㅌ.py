...
# 2. 페이지 상단 레이아웃
st.title("📊 산업별 거시경제 스트레스 테스트")
st.caption("Page 1: Shock & Resilience 리스크 진단 화면")
st.markdown("""
특정 업종을 선택하여 외부 충격(2020년 코로나 팬데믹, 2021년 자산 과열기)이 발생했을 때 
해당 산업의 창업 생태계가 얼마나 흔들렸는지 **변동성(Volatility)**을 측정하고 재무 리스크를 분석합니다.
""")
st.markdown("---")
...
# 6. 양대 인터랙티브 데이터 시각화 차트 배치 (시각성 극대화)
...
if abs(shock_2020) >= 15 or abs(shock_2021) >= 15:
    ...
    - **재무적 제언:** 경기 하강 국면에서 변동성이 극대화되어 초기 투자금 회수 리스크가 커질 위험이 높습니다. 고정비(임대료, 고정 인건비) 비율을 최소화하고, 경영난에 대비해 최소 3~6개월을 버틸 수 있는 **비상 운전자본(Cash Buffer)**을 일반 업종보다 두텁게 확보해야 지속가능성(Going Concern)을 유지할 수 있습니다.
    """)
else:
